...
?>
<!-- Step 1: html -->
<div ref="chartContainer" id="your-analytics-active-hours-<?=$args['id'] ?>"></div>
...
<!-- FOR DEMO ONLY. DO NOT COPY -->
	...
	<script src="<?=FWAPPS_ROOT_URL.'/app-'.FWAPPS_APP; ?>/assets/scripts/charts/insights-your-analytics/d3ActiveHours.mine.js"></script>
	...
	*/
	...
	// IIFE
	(function(d3ActiveHours,demo_only,window){

	///////////////////// STEP 2 -  Setup
	
	
		//@REPLACE `document.getElementById('#your-analytics-allocation-<?=$args['id'] ?>')` to match this.$refs.chartContainer
		const chartContainer = document.getElementById('your-analytics-active-hours-<?=$args['id'] ?>'); 
	
		const chart = new d3ActiveHours(chartContainer);
		...
		*/
		...
		const dataDayDiff = d3.timeHour.count(
			tMin,
			d3.min([lMax,tMax])
		) / demo_only.TheHours.length;


		const dataSplitByHourTotal =  Object.values(
			demo_only.DataSplitByHour(demo_only.FilterData(
				timeByMode,
				_data,
				latestDate,
				startDate
				...
				.reduce((acc, d) => {
					const date = demo_only.DateToObj([d.date, demo_only.FloatToTime(Math.floor( demo_only.TimeToFloat(d.start_time)))]);
					...
					if (!acc[date]) {
						acc[date] = {
							...
							start_time:  demo_only.FloatToTime( Math.floor( demo_only.TimeToFloat(d.start_time)) )
						};
					} else {
						acc[date].total_hours =  demo_only.FloatToTime(  demo_only.TimeToFloat(acc[date].total_hours) +  demo_only.TimeToFloat(d.total_hours) );
					}
			...
			demo_only.TheHours.map(hour => {
				const entries = dataSplitByHourTotal
					.filter( d => Math.floor(demo_only.TimeToFloat(d.start_time)) == demo_only.TimeToFloat(hour) )
					;
				...
				if(demo_only.TimeToFloat(avg) > 0 ){
					newData[Math.floor(demo_only.TimeToFloat(hour))] = demo_only.TimeToFloat(avg);
				}
			});

			args.data = newData;

			console.warn('active hours replicate crData',args.data);
	...
	}(window.d3ActiveHours,window.demo_only,window));
