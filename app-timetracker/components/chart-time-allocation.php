...
?>
<!-- Step 1: html -->
<div ref="chartContainer" id="your-analytics-allocation-<?=$args['id'] ?>"></div>

<!-- FOR DEMO ONLY. DO NOT COPY -->
	<script src="https://d3js.org/d3.v7.min.js"></script>
	<script src="<?=FWAPPS_ROOT_URL.'/app-'.FWAPPS_APP; ?>/assets/scripts/charts/insights-your-analytics/d3Allocation.mine.js"></script>
	...
	*/
	...
	// IIFE
	(function(d3Allocation,demo_only,window){

	///////////////////// STEP 2 -  Setup
	
	
		//@REPLACE `document.getElementById('#your-analytics-allocation-<?=$args['id'] ?>')` to match this.$refs.chartContainer
		const chartContainer = document.getElementById('your-analytics-allocation-<?=$args['id'] ?>'); 
	
		const chart = new d3Allocation(chartContainer);
		...
		*/
		...
		function DEMOToggleChart(e) {
			...
			newData = demo_only.FilterData(
				...
				acc[d[meta]] += demo_only.StringToMins(d.total_hours); // @TODO make 0 to 1 data
				
				return acc;
			}, {});

			totalSum = d3.sum(Object.keys(newData), m => newData[m] );

			Object.keys(newData).forEach(m => {
				newData[m] = newData[m] / totalSum;
			})

			args.data = {...newData};

			console.warn('Allocations replicate crData',args.data);
	...
	}(window.d3Allocation,window.demo_only,window));
