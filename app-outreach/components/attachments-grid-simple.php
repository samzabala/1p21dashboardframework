

<ul class="attachment-list list-group list-group-interactive">
	<li>
		<div class="flex-grid justify-content-space-between align-items-center flex-grid-fixed flex-grid-no-gutter-y">
			<div class="flex-col-9">

				<span class="attachement-filename text-wrap-ellipsis">
					<span class="REPLACE">filename.virus</span>
				</span>

				<span class="attachement-filezise text-wrap-ellipsis font-style-italic color-neutral">
					<span class="REPLACE">69gb</span>
				</span>

			</div>
			<div class="flex-col-3 text-align-right">

				<a class="color-primary color-secondary-hover" href="#" onclick="REPLACEwithFnToFeleteAttachment"><i class="symbol symbol-delete"></i></a>
			</div>
		</div>
	</li>

	<!-- placeholder -->
	<?php for($i=1; $i<10; $i++){ ?>

		<li>
			<div class="flex-grid justify-content-space-between align-items-center flex-grid-fixed flex-grid-no-gutter-y">
				<div class="flex-col-9">

					<span class="attachement-filename text-wrap-ellipsis">
						<span class="REPLACE">filename.virus</span>
					</span>

					<span class="attachement-filezise text-wrap-ellipsis font-style-italic color-neutral">
						<span class="REPLACE">69gb</span>
					</span>

				</div>
				<div class="flex-col-3 text-align-right">

					<a class="color-primary color-secondary-hover" href="#" onclick="REPLACEwithFnToDeleteAttachment"><i class="symbol symbol-delete"></i></a>
				</div>
			</div>
		</li>
	<?php } ?>
</ul>